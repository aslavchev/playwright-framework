...
test.describe('Repos API', () => {
    test('Valid owner + valid repo return the repo name',{tag:'@smoke'}, async ({ apiRequest }) => {
    ...
    test('Get Non-existent repo return 404',{tag:'@smoke'}, async ({ apiRequest  }) => {
        const { status, body } = await apiRequest({
            method:'GET',
    ...
    test('List user repos returns array of repos',{tag:'@smoke'}, async ({ apiRequest}) => {
        const { status, body } = await apiRequest({
            method:'GET',
            url: '/user/repos',
         }); 
