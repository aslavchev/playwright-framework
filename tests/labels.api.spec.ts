...
test.describe('Labels API', () => {
    ...
    test('CRUD label lifecycle', {tag: '@regression'}, async ({ apiRequest }) => {
    ...
    test('Create duplicate label returns 422', {tag: '@regression'}, async ({ apiRequest }) => {
        const { status } = await apiRequest({
            method: 'POST',
            url: repoPath,
    ...
    test('Delete non-existent label returns 404', {tag: '@regression'}, async ({ apiRequest }) => {
        const { status } = await apiRequest({
            method: 'DELETE',
